...
We can leverage the same [path traversal attack](https://owasp.org/www-community/attacks/Path_Traversal) as before by issuing a request to `https://tag-generator.kringlecastle.com/image?id=../../../app/lib/app.rb` to download [`app.rb`](../artifacts/objectives/o8/app.rb). Reviewing the code, we can see that the web application uses a `system()` call to resize the uploaded image to a specific size (line 7). Because the backend Ruby code does not filter the `filename` variable used as input to the `system()` call, we can leverage this flaw to execute our own code.
