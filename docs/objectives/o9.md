...
Together with the ARP and DNS requests, we're now also redirecting some of Jack's HTTP traffic to our machine.

![Packet capture 3](../img/objectives/o9/packet_capture_3.png)

Running a simple Python web server using `python3 -m http.server 80` will tell us that Jack's hijacked machine is trying to access `/pub/jfrost/backdoor/suriv_amd64.deb`. Let's host a [malicious payload](http://www.wannescolman.be/?p=98) there. :smiling_imp:


### Create the payload

One of the `.deb` files on our machine is `netcat-traditional_1.10-41.1ubuntu1_amd64.deb` which has both a `postinst` file that can hold our payload and will also install the `nc` binary we'll use to establish the [reverse shell](https://highon.coffee/blog/reverse-shell-cheat-sheet/) connection back to our machine.

To help remove some of the tedious packaging and setup work create a shell script that unpacks `netcat-traditional_1.10-41.1ubuntu1_amd64.deb` (lines 9-13), adds the `nc` reverse shell command to the `postinst` file (line 18), packages everything back up (line 20), creates the proper directory structure for the HTTP request to succeed (line 22), and starts a Python web server (line 27).
...
Now that all the required components are in place, open 3 additional Tmux windows (for a total of 4). One shell should already be running the Python webserver. Use the remaining windows to start a netcat listener with `nc -nlvp 8888` to accept the incoming reverse shell connection, the updated [`dns_resp.py`](../artifacts/objectives/o9/dns_resp.py) script, and finally the modified [`arp_resp.py`](../tools/objectives/../o9/arp_resp.py) script.
